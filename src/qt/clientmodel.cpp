...
void ClientModel::updateAlert(const QString &hash, int status)
{
    ...
    if(status == CT_NEW)
    {
        ...
        if(!alert.IsNull())
        {
            emit error(tr("Network Alert"), QString::fromStdString(alert.strStatusBHC), false);
        }
    }

    // Emit a numBlocksChanged when the status message changes,
    // so that the view recomputes and updates the status BHC.
...
QString ClientModel::getStatusBHCWarnings() const
{
    return QString::fromStdString(GetWarnings("statusBHC"));
}
